...
parser.add_argument('--feature_name', help='what type of feature to use', default='IDT')
args = parser.parse_args()

feature_name = args.feature_name # IDT, CLIP_64, CLIP_256, SAM_DINO_64, SAM_DINO_128, SAM_DINO_256
...
if p['db_name'] == 'breakfast_action':
    ...
    features_dataset = FeaturesDataset(p['consider_background'], p['subset'], feature_name)
...
for video_idx, sample in enumerate(features_dataset_np):
    ...
    similarity_matrix_titles.append(feature_name)
    ...
    for i, cluster_technique in enumerate(cluster_techniques):
        ...
        if not p['consider_background']:
            ...
            save_segmentation_plot(labels_gt_bg, [categorical_labels_idt_bg, categorical_labels_tsa_bg], 
                                   os.path.join(output_path, '{}.segmentation_{}.png'.format(filename, cluster_technique)), features_types=[feature_name, "TSA"])
        else:
        ...
        if p['tsne_plot']:
            ...
            save_multiple_tsne(tsa_features_tsne, categorical_labels_tsa, labels_gt,
                               tsne_path=os.path.join(output_path, f'{filename}.tsne_tsa_{cluster_technique}.png'), tsne_title="TSA Features")
            save_multiple_tsne(init_features_tsne, categorical_labels_idt, labels_gt,
                               tsne_path=os.path.join(output_path, f'{filename}.tsne_{feature_name}_{cluster_technique}.png'), tsne_title=f"{feature_name} Features")
